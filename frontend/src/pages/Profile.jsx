...
const Profile = () => {
  ...
  const [isViewOnly, setIsViewOnly] = useState(false) // Flag for view-only mode (viewing other employees)
  const [viewDocument, setViewDocument] = useState(null) // State for document viewer modal

  useEffect(() => {
    // Always fetch team members if manager (needed for dropdown)
    if (user?.role === 'manager') {
      console.log('[PROFILE] Manager detected, fetching team members...', { userId: user._id, employeeId: user.employeeId, role: user.role })
      fetchTeamMembers()
    }
    // Note: Only log for managers to reduce console clutter for employees
    
    // Fetch profile based on employeeId parameter
    if (employeeId) {
      // Viewing specific employee profile (view-only for managers)
      fetchEmployeeProfile(employeeId)
      setIsViewOnly(true)
      setEditing(false) // Always disable editing when viewing other employees
    } else {
      // Viewing own profile
  ...
  const fetchMissingFields = async () => {
    try {
      const { data } = await axios.get('/api/profile/completion-status')
  ...
  // Auto-enable editing mode for employees - they need to fill their own profile
  // Disable editing if viewing other employees' profiles (view-only mode)
  useEffect(() => {
    if (isViewOnly) {
      setEditing(false)
      return
    ...
    if (profile && user?.role === 'employee') {
      // Always enable editing for employees so they can fill their profile
    ...
    try {
      ...
      if (forceRecalculate) {
        try {
          ...
          console.error('Error recalculating profile:', recalcError)
        }
      }
      ...
      if (autoCreate && (error.response?.status === 404 || error.response?.status === 500 || !profile)) {
        ...
        try {
          ...
          const { data: createdData } = await axios.post('/api/employees/create-profile')
          if (createdData?.employee) {
            // Wait a moment then retry fetching profile
            setTimeout(async () => {
              try {
                const { data: profileData } = await axios.get('/api/profile/my-profile')
                ...
                toast.error('Profile created but failed to load. Please refresh the page.')
              } finally {
                setLoading(false)
              }
            }, 500)
          ...
          const createErrorMessage = createError.response?.data?.message || createError.response?.data?.error || 'Failed to create profile'
          console.error('Create error details:', createError.response?.data)
          // Don't show error toast here - let it show the "Create My Profile" button instead
        }
      }
  ...
  // Fetch employee profile by ID (for managers viewing team members)
  const fetchEmployeeProfile = async (empId) => {
    ...
    try {
      ...
      setIsViewOnly(data.isViewOnly || false)
      setEditing(false) // Always disable editing when viewing other employees
      setLoading(false)
    } catch (error) {
  ...
  const fetchTeamMembers = async () => {
    try {
      console.log('[PROFILE] Fetching team members for manager...')
      const { data } = await axios.get('/api/employees')
      console.log('[PROFILE] ✅ Received team members from database:', {
        count: data?.length || 0,
        rawData: data,
        firstMember: data?.[0] ? {
          id: data[0]._id,
          employeeId: data[0].employeeId,
          name: data[0].personalInfo?.fullName,
          email: data[0].personalInfo?.email,
          department: data[0].companyDetails?.department
        } : null
      })
      
      // Filter out the manager's own profile from team members list
      // (manager's own profile is accessed via "My Profile" option)
      const filteredTeamMembers = (data || []).filter(member => {
        // Exclude if it's the manager's own profile
        const isOwnProfile = member._id === user?.employeeId || member._id?.toString() === user?.employeeId?.toString();
        return !isOwnProfile;
      })
      
      console.log('[PROFILE] ✅ Filtered team members from database (excluding own profile):', {
        count: filteredTeamMembers.length,
        members: filteredTeamMembers.map(m => ({
          id: m._id,
          name: m.personalInfo?.fullName || 'Unknown',
      ...
      if (filteredTeamMembers.length === 0) {
        console.warn('[PROFILE] No team members found. This could mean:')
        console.warn('  1. No employees have reportingManager set to this manager')
        console.warn('  2. Manager profile might not be properly linked')
        console.warn('  3. All team members might be inactive')
      }
    } catch (error) {
      console.error('[PROFILE] Error fetching team members:', error)
  ...
  const fetchTimeline = async () => {
    try {
      const timelineEndpoint = employeeId ? `/api/profile/timeline?employeeId=${employeeId}` : '/api/profile/timeline'
  ...
  const handleSubmitProfile = async () => {
    ...
    setSubmitting(true)
    try {
      const { data } = await axios.post('/api/profile/submit')
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (dataToSend.idProofs.aadhaar !== undefined && dataToSend.idProofs.aadhaar !== null && String(dataToSend.idProofs.aadhaar).trim() === '') {
        dataToSend.idProofs.aadhaar = ''
      }
      if (dataToSend.idProofs.pan !== undefined && dataToSend.idProofs.pan !== null && String(dataToSend.idProofs.pan).trim() === '') {
        dataToSend.idProofs.pan = ''
      }
      if (dataToSend.idProofs.passport !== undefined && dataToSend.idProofs.passport !== null && String(dataToSend.idProofs.passport).trim() === '') {
        dataToSend.idProofs.passport = ''
      }
      if (dataToSend.idProofs.drivingLicense !== undefined && dataToSend.idProofs.drivingLicense !== null && String(dataToSend.idProofs.drivingLicense).trim() === '') {
      ...
      // Keep all bank detail fields
      if (dataToSend.bankDetails.accountNumber !== undefined && dataToSend.bankDetails.accountNumber !== null && String(dataToSend.bankDetails.accountNumber).trim() === '') {
        dataToSend.bankDetails.accountNumber = ''
      }
      if (dataToSend.bankDetails.ifscCode !== undefined && dataToSend.bankDetails.ifscCode !== null && String(dataToSend.bankDetails.ifscCode).trim() === '') {
        dataToSend.bankDetails.ifscCode = ''
      }
      if (dataToSend.bankDetails.bankName !== undefined && dataToSend.bankDetails.bankName !== null && String(dataToSend.bankDetails.bankName).trim() === '') {
        dataToSend.bankDetails.bankName = ''
      }
      if (dataToSend.bankDetails.branchName !== undefined && dataToSend.bankDetails.branchName !== null && String(dataToSend.bankDetails.branchName).trim() === '') {
        dataToSend.bankDetails.branchName = ''
      }
      if (dataToSend.bankDetails.accountHolderName !== undefined && dataToSend.bankDetails.accountHolderName !== null && String(dataToSend.bankDetails.accountHolderName).trim() === '') {
      ...
      const { data } = await axios.put('/api/profile/my-profile', dataToSend)
      
      // Update state with response data
      setProfile(data)
      setFormData(data) // Update formData with latest data (including decrypted values)
      setEditing(false)
      ...
      console.error('[PROFILE_UPDATE] Error:', error.response?.data || error.message)
  ...
  const handleFileUpload = async (e, documentType, companyIndex = null, documentCategory = null) => {
    ...
    // If uploading for a company, check if the company exists in the database
    if (companyIndex !== null && documentCategory) {
      const companies = profile?.employmentInfo?.previousCompanies || []
      if (!companies[companyIndex]) {
        toast.error('Please save the company information first before uploading documents. Click "Save Changes" after adding the company.')
        e.target.value = '' // Clear the file input
        return
      }
      
      // Check if company has at least a name (basic validation)
      const company = companies[companyIndex]
      if (!company?.companyName || (company.companyName && String(company.companyName).trim() === '')) {
        toast.error('Please enter the company name and save it first before uploading documents.')
        e.target.value = '' // Clear the file input
        return
      }
    }
    ...
    setUploading(true)
    try {
      const { data } = await axios.post('/api/profile/upload-document', uploadFormData, {
      ...
      if (error.response?.data?.message) {
        ...
        errorMessage = 'Company not found. Please save the company information first.'
      ...
      // If it's a company document error, provide more context
      if (companyIndex !== null && documentCategory) {
        console.error('[UPLOAD] Company document upload failed:', {
          companyIndex,
          documentCategory,
          error: errorMessage,
  ...
  if (loading && !profile) {
    return (
      <div className="space-y-6">
        {/* Team Member Selector for Managers - Show even during loading */}
        {user?.role === 'manager' && (
          <motion.div
  ...
  const handleCreateProfile = async () => {
    ...
    try {
      const { data } = await axios.post('/api/employees/create-profile')
      if (data?.employee) {
        toast.success('Profile created successfully!')
        // Wait a moment then fetch profile
        setTimeout(async () => {
          await fetchProfile()
        }, 500)
      } else {
      ...
      toast.error(`${errorMessage}${errorDetails ? ` - ${errorDetails}` : ''}`, { duration: 5000 })
    } finally {
  ...
  if (!profile && !loading && !creatingProfile) {
    return (
      <div className="space-y-6">
        {/* Team Member Selector for Managers - Show even when profile not found */}
        {user?.role === 'manager' && (
          <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex-1 min-w-[300px]">
                ...
                <p className="text-xs text-gray-600 mt-2">
                  <span className="text-orange-600 font-medium">
                    ⚠️ Your profile not found. Create it below or view team members above.
                  </span>
                </p>
              </div>
            </div>
        ...
        <div className="text-center py-12">
          <div className="max-w-md mx-auto">
            ...
            <p className="text-gray-600 mb-6">
              Your employee profile hasn't been created yet. Click the button below to create it automatically.
            </p>
            <button
  ...
  return (
    <div className="space-y-6">
      {/* Team Member Selector for Managers - Always show for managers */}
      {user?.role === 'manager' && (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex-1 min-w-[300px]">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                <FiUsers className="inline mr-2 text-blue-600" size={18} />
                {isViewOnly ? 'Viewing Team Member Profile' : 'View Employee Profile'}
              </label>
              <select
                value={employeeId || 'my-profile'}
                onChange={(e) => {
                  if (e.target.value === 'my-profile') {
                    ...
                    navigate(`/profile/${e.target.value}`)
                    setIsViewOnly(true)
                    setEditing(false) // Disable editing when viewing team member
                  }
                }}
                className="input w-full bg-white border-2 border-blue-300 focus:border-blue-500"
                disabled={loading}
              >
                ...
                {teamMembers && teamMembers.length > 0 ? (
                  teamMembers.map((member) => {
                    // Display employee name from database, fallback to employeeId, then Unknown
                  ...
                  <option value="" disabled>No team members found in database</option>
                )}
              </select>
              <p className="text-xs text-gray-600 mt-2">
                {isViewOnly ? (
                  <span className="text-blue-700">
                    📖 <strong>View-only mode:</strong> You can view but not edit this employee's profile
                  </span>
                ) : (
                  <span className="text-green-700">
                    ✏️ <strong>Edit mode:</strong> You can edit your own profile
                  </span>
                )}
                {teamMembers.length === 0 && !loading && (
                  <span className="block text-orange-600 mt-1 font-medium">
                    ⚠️ No team members assigned yet. Contact HR to assign team members to you.
                  </span>
                )}
                {loading && (
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              {profile.personalInfo?.fullName 
                ? (profile.personalInfo?.fullName && String(profile.personalInfo.fullName).trim()) || 'My Profile'
                : 'My Profile'}
            </h1>
            <p className="text-gray-600 mt-1">Employee ID: {profile.employeeId}</p>
          ...
          {isViewOnly && (
            <div className="flex items-center px-4 py-2 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <span className="text-sm font-medium text-blue-800">View Only</span>
            </div>
          )}
          {editing && (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-center justify-between p-4 border-b">
              <h3 className="text-lg font-semibold text-gray-900">{viewDocument.title}</h3>
              <button
            ...
            <div className="flex-1 overflow-auto p-4">
              {viewDocument.url.endsWith('.pdf') || viewDocument.url.includes('.pdf') ? (
                <iframe
                  src={`http://localhost:5000${viewDocument.url}`}
            ...
            <div className="p-4 border-t flex items-center justify-between bg-gray-50">
              <a
                href={`http://localhost:5000${viewDocument.url}`}
                target="_blank"
    ...
    <div className="space-y-6">
        ...
        <div className="md:col-span-2">
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <p className="text-xl font-bold text-green-700">
                  {profile.companyDetails?.monthlySalary 
                    ? `₹${profile.companyDetails.monthlySalary.toLocaleString('en-IN')}`
                    : 'Not Set'
                  }
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-600 mb-1">Salary Status</label>
                <span className={`inline-flex px-3 py-1 rounded-full text-sm font-semibold ${
                  profile.companyDetails?.salaryStatus === 'Active' 
                    ? 'bg-green-100 text-green-800'
                    : profile.companyDetails?.salaryStatus === 'On Hold'
              ...
              {profile.companyDetails?.salaryLastUpdated && (
                <div>
                  <label className="block text-sm font-medium text-gray-600 mb-1">Last Updated</label>
                  <p className="text-sm text-gray-700">
                    {new Date(profile.companyDetails.salaryLastUpdated).toLocaleDateString('en-GB', {
                      day: '2-digit',
                      month: 'short',
                      year: 'numeric'
                    })}
            ...
            <p className="text-xs text-gray-500 mt-3">
              Salary information is managed by HR. Contact HR for any salary-related queries.
            </p>
          </div>
        </div>
      </div>
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-sm text-blue-800">
          <FiAlertCircle className="inline mr-2" />
          Joining Date, Employment Status, and Salary are managed by HR and cannot be changed by employees.
        </p>
      </div>
...
const EmploymentTab = ({ profile, formData, setFormData, editing, onSave, onCancel, onUpload, uploading, refreshKey = 0, onViewDocument }) => {
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => onViewDocument && onViewDocument({
                                url: expLetterUrl,
                                title: `Experience Letter - ${company.companyName || 'Previous Company'}`
                              })}
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-1 px-3 py-2 bg-blue-50 text-blue-700 border border-blue-300 rounded-lg hover:bg-blue-100 hover:border-blue-400 transition-colors text-sm font-medium"
                        ...
                        {salarySlipUrl ? (
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => onViewDocument && onViewDocument({
                                url: salarySlipUrl,
                                title: `Salary Slip - ${company.companyName || 'Previous Company'}`
                              })}
                              className="inline-flex items-center gap-2 px-4 py-2 bg-green-50 text-green-700 border border-green-300 rounded-lg hover:bg-green-100 hover:border-green-400 transition-colors text-sm font-medium shadow-sm"
...
const DocumentsTab = ({ profile, onUpload, uploading }) => {
